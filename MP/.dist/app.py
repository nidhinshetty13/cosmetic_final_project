...
def check_product(product_name, skin_type, top_n=3):
    try:
        ...
        match = df.loc[df["product_name"] == product_name]
        if match.empty:
            return (
        ...
        toxicity_value = int(product[f"toxicity_{skin_type}"])
        ...
        if toxicity_value == 0:
            ...
            message = f"❌ {icon} <b>{product['product_name']}</b> is <b>TOXIC</b> for <b>{skin_type}</b> skin."
            prod_vec = vectorizer.transform([product["ingredients"]])
            similarities = (X @ prod_vec.T).toarray().ravel()
            temp = df.copy()
            temp["similarity"] = similarities

            same_cat = temp["category"].str.lower() == str(product["category"]).lower()
            safe_mask = temp[f"toxicity_{skin_type}"] == 0
